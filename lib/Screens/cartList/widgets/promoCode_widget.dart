...
class PromocodeWidget extends StatelessWidget {
        ...
        return Container(
          ...
          child: ListView(
            children: [
              ...
              //add promotion code text Select a promotion or enter a promo code
              const Text(
                'Select a promotion or enter a promo code',
                style: TextStyle(fontSize: 14, color: Colors.black),
              ),
              SizedBox(height: 10.h),
              controller.response.membership == null ||
                      controller.response.membership!.membershipId.toString() ==
                          "null"
                  ? SizedBox.shrink()
                  : Container(
                      padding: const EdgeInsets.all(10),
              ...
              rewardMethod(controller, reward_id),
              //create promocode list
              promoCodeList(controller, selectedId, context),
  ...
  Column rewardMethod(CartController controller, reward_id) {
    return Column(
      children: List.generate(controller.availableData.length, (index) {
        bool isSelected = controller.availableData[index].isApplied!;
        return Container(
          padding: const EdgeInsets.all(10),
          margin: const EdgeInsets.only(bottom: 10),
              ...
              TextButton(
                onPressed: () {
                  if (isSelected) {
                    ...
                    //get cartid index
                    controller.rewardId = controller
                        .availableData[index]
                        .rewardId
                        .toString();
                    controller.rewardName = controller
  ...
  Column promoCodeList(CartController controller, selectedId, context) {
    return Column(
      children: List.generate(controller.couponAvailableData.length, (index) {
        // ✅ Use isApplied directly
        bool isApplied =
              ...
              TextButton(
                onPressed: () async {
                  if (isApplied) {
                    ...
                    controller.promoname =
                        controller.couponAvailableData[index].title;
                    controller.promoId = controller
                        .couponAvailableData[index]
                        .id
                        .toString();
                    controller.selectPromoCode(
                      controller.couponAvailableData[index].promoCode,
